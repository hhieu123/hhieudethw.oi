...
var login = require("fca-tecna"), moment = require("moment-timezone");
var { readFileSync } = require("fs-extra"), axios = require("axios");
log("🧚‍♀Tecna bot🧚‍♀", "Đang khởi tạo các biến...");

var tecna = new Object({
    configs: [],
    log: log,
    superBan: new Map(),
    getTime: function (option) {
        switch (option) {
            case "seconds":
                return `${moment.tz("asia/quy_nhon").format("ss")}`;
            case "minutes":
                return `${moment.tz("asia/quy_nhon").format("mm")}`;
            case "hours":
                return `${moment.tz("asia/quy_nhon").format("HH")}`;
            case "date": 
                return `${moment.tz("asia/quy_nhon").format("DD")}`;
            case "month":
                return `${moment.tz("asia/quy_nhon").format("MM")}`;
            case "year":
                return `${moment.tz("asia/quy_nhon").format("YYYY")}`;
            case "fullHour":
                return `${moment.tz("asia/quy_nhon").format("HH:mm:ss")}`;
            case "fullYear":
                return `${moment.tz("asia/quy_nhon").format("DD/MM/YYYY")}`;
            case "fullTime":
                return `${moment.tz("asia/quy_nhon").format("HH:mm:ss DD/MM/YYYY")}`;
        }
    },
    host: []
});
...
log("🧚‍♀Tecna bot🧚‍♀", "💸Đang kiểm tra thông tin và các cài đặt cần thiết...");

require("./handle/loadAll") ({ tecna, multiple });

log("🧚‍♀Tecna bot🧚‍♀", "🐸Đang tiến hành đăng nhập...");
var { optionsLogin } = tecna.configs;

try {
    var dataLogin = JSON.parse(readFileSync('./fbstate.json', 'utf8'));
} catch {
    log("LOGIN", "💗Không tìm thấy file fbstate.json - Vui lòng kiểm tra và thử lại.", "error");
}
